...
void FractionalPacking::iteration() {
    ...
    if(flow_change){
        if(time_debug){
            cout<<flow_x_i.size()<<" "<<old_fxi.size()<<endl;
        }
        /*
...
double FractionalPacking::compute_theta_newton_raphson(vector<double> x0, vector<double> x1, double theta0 = 0.5,
    ...
    try{
        while(new_theta<2&&new_theta>-1 &&abs(new_theta - theta)>1e-4){
            theta = new_theta;
...
double FractionalPacking::update_theta(double theta, vector<double> u_0, vector<double> u_1) {
    ...
    for(int i = 0; i< u_0.size(); i++){
        ...
        /*
        if((1-theta*u_0[i]+theta*u_1[i])<0){
            cout<<"ui<0: "<<i<< " "<<u_0[i]<<" "<< u_1[i]<<endl;
        }
         */
        f_theta0 += t[i];
    ...
    //cout<<"oldtheta: "<<theta<<" new_theta: "<<theta - f_theta0/fprime_theta0<<endl;
